...
Tests: `python tests/smoke_test.py` (118 offline source code checks) + `python tests/test_logic.py` (173 unit tests with mocked DB/WS/Telegram — tests entry rejections, dynamic pricing, binary risk filter, blocked question keywords, rapid drop, resolution, MAX_LOSS, sim costs, quality scoring sweet spot, theme quality adjustment, shared utilities, rapid drop block counter, REST poll stale positions, price history throttle) + `python tests/test_bugfixes.py` (63 regression tests — WS multi-key, NO price, year rollover, HTML escaping, WS book events, shared utilities, theme classification, CLOB book parser, SL blacklist max_loss, rest_poll non-clobber, MAX_LOSS bypass config). No linter configured. Logging to stdout. Deployed via Railway (`Procfile: worker: python main.py`).
...
- **engine/monitor.py** (~330 lines) — Position monitoring via WS. Exit priority: resolution → early TP → MAX_LOSS → rapid drop. `_verify_price_and_volume(http_client, market_id, side, token_id)` — fetches **CLOB book** (live bid/ask from `clob.polymarket.com/book`) in parallel with Gamma `/markets/{id}` (volume + acceptingOrders). For NO side: `NO_bid = 1 - YES_ask`. Falls back to Gamma midpoint if CLOB unavailable. Tenacity retry on timeout/network error. `_do_close()` — shared atomic close + WS cleanup + theme recalibration. `_tg_base()` — shared telegram message builder. No percentage SL — only MAX_LOSS + rapid drop. RAPID_DROP_PCT from config. REST cooldown 60s per market. Volume confirmation for rapid drop. **Rapid drop block counter** (`_rapid_drop_blocks`): after 3 consecutive REST blocks, bypasses REST verification and trusts WS bid directly. **MAX_LOSS block counter** (`_max_loss_blocks`): after `MAX_LOSS_BYPASS_BLOCKS` (default 2, config_live) blocks, trust WS bid; counter also bumped from rest_poll when REST shows loss > cap (so silent-WS positions reach bypass in finite time). **MAX_LOSS_BLOCKED telemetry**: when REST blocks a max_loss close, the REST price is written to `micro_price_history` with `source='max_loss_blocked'` for diagnostics. **REST poll safety net** (`rest_poll_stale_positions`): called every scan cycle, fetches CLOB price for positions where WS has been silent ≥5 min, passes price explicitly via `info` dict to `check_position_price` — does NOT overwrite `ws.prices[ws_key]['best_bid']` (preserves WS-observed drops); only refreshes `last_update`. **Price history recording**: on each WS tick, records to `micro_price_history` when price moves ≥0.3¢ or ≥30s since last record (throttled).
...
- **engine/scanner.py** (~590 lines) — Fetches up to 1600 markets from Gamma API (16 pages, parallel). `dynamic_entry_price()`: ≤1d→90¢, ≤2d→92¢, ≤3d→93¢, >3d→base (configurable via ENTRY_PRICE_1D/2D/3D). `is_binary_risk()` filter blocks markets that can lose entire stake instantly: range bets ("between $X and $Y"), coin flips ("Up or Down", "Green or Red"), per-game/per-map esports ("Game 2 Winner"), soccer draw bets ("end in a draw"), crypto price-level markets ("Bitcoin above $76k", "BTC dip to $74k", "ETH reach $3,500" — flash-crash risk; covers above/over/exceed/reach/hit/surpass AND below/under/drop/fall/dip). `BLOCKED_QUESTION_KEYWORDS` + `is_blocked_question()` — permanent per-keyword blacklist ("hong kong", "cape town", "global temperature"), checked in scanner loop AND in `try_enter()` to cover event cascade. Filters: volume >$50k, spread <2¢, ≤7 days to expiry. Both YES and NO sides checked. Quality scoring (0-100). Price ceiling: both sides >98¢ skipped (ROI too low after costs). Theme classification (~30 themes, **esports BEFORE sports** in dict — critical for keyword priority, `_VS_PATTERN` regex fallback handles FC/BSC/AFC abbreviations, `_WIN_ON_DATE` pattern). Date parsing from question text with year-rollover handling. Event siblings map for negRisk cascade.
...
- **Entry Logic**: Buy YES/NO at best_ask price. Stake = `PCT_STAKE` of bankroll (default 5%), dynamically capped by time-to-expiry: Q≥80 + ≤6h → `MAX_STAKE_Q80_6H=$75` with 7.5% Kelly; Q≥80 + ≤1d → `MAX_STAKE_Q80_1D=$50` with 7.5% Kelly (middle tier); ≤6h → `MAX_STAKE_6H=$50`; ≤1d → `MAX_STAKE_1D=$35`; >1d → `MAX_STAKE=$20`. ROI at resolution must be ≥1.8%. Esports always uses base `MAX_STAKE` regardless of quality/time (flash-crash risk). Production data: Q80+ has near-100% WR — was undercapitalized at base 5% pre-fix.
...
- **Rapid Drop Block Counter**: After 3 consecutive REST blocks, bypasses REST and trusts WS bid directly. Counter resets when price recovers above the threshold or position closes. (Less critical now that REST verify uses CLOB book — but kept as a safety net for the rare case where CLOB also lags.)
- **MAX_LOSS Block Counter**: Same idea but threshold is `MAX_LOSS_BYPASS_BLOCKS` (config_live, default 2 — kept low because the cap is small and stale-state tolerance must be tight). `rest_poll_stale_positions` also bumps the counter when REST already shows a loss past the cap, so a silent-WS position reaches bypass after a few poll cycles instead of being stuck forever waiting for WS ticks. On every block, the REST price is written to `micro_price_history` with `source='max_loss_blocked'` for diagnostics.
...
- **Theme Management**: Managed entirely via dashboard (block/unblock) + Bayesian auto-block (shrinkage k=20, block if adj WR < 40% after 5+ trades). No hardcoded blocked themes on startup. Max 3 positions per theme.
...
- **REST PnL on Exit**: Rapid drop and MAX_LOSS use REST-verified price (CLOB book) for PnL calculation when available. CLOB returns the YES token's orderbook; for NO positions, the bid we'd realize is `1 - YES_ask`. If CLOB is unreachable, `_verify_price_and_volume` falls back to Gamma midpoint via `shared.parse_outcome_prices()`.
- **Theme Classification**: ~30 themes with keyword matching. **Esports dict entry is BEFORE sports** — critical, otherwise esports tournament keywords ("world cup", "playoff", "masters") match sports first. Handles: CBA basketball teams, Chinese soccer clubs (Shenhua, Haigang), tennis venues (Linz, Monza, Barletta). `_VS_PATTERN` regex supports abbreviations (FC, BSC, AFC). Tech keyword `" ai "` (with spaces) avoids false positives on "Shanghai". Note: "end in a draw" is in sports keywords list but is also caught by `is_binary_risk()` — never enters.
...
- **Binary risk filter**: Blocks markets where price jumps to 0 instantly: range bets ("between $X and $Y"), coin flips ("Up or Down", "Green or Red", "Higher or Lower"), per-game/per-map esports winner markets ("Game 2 Winner", "Map 3 Winner") — a single live game resolves in 15-40 min and can flip from 95¢+ to $0 on a single upset. Soccer "end in a draw" bets — binary Win/Draw/Loss outcome. Crypto price-level markets in BOTH directions: upward targets ("Bitcoin above $76k", "BTC reach $80k", "ETH exceed $3,500") and downward targets ("BTC dip to $74k", "ETH drop to $2,250", "Bitcoin below $70k") — BTC/ETH can flash-crash through a price target in minutes, identical risk profile to esports per-game. Production data (April 2026): all 6 BTC "dip to $X" markets hit max_loss for ~$25 in 30d before this filter — `dip to / dips to` was added to the regex after this. BO3/BO5 series winner markets are allowed.
- **Blocked question keywords** (`BLOCKED_QUESTION_KEYWORDS` in scanner.py + checked in `try_enter()`): Permanent per-keyword blacklist — "hong kong", "cape town", "global temperature". Checked both in scanner loop AND inside `try_enter()` to block event cascade entries.
- **Theme blocking**: Managed entirely via dashboard. Bayesian auto-block for themes with WR < 40% after 5+ trades (`BLOCK_MIN_TRADES=5`, lowered from 10 — faster reaction on toxic themes). No hardcoded defaults.
- **Quality gate**: Dynamic threshold (≤1d→Q≥40, 1-3d→Q≥55, 3-5d→Q≥70, 5d+→Q≥80)
- **MAX_LOSS hard cap**: Default $3 per position, always enforced. REST-verified via CLOB book (live orderbook). Bypass after `MAX_LOSS_BYPASS_BLOCKS` (default 2) consecutive REST disagreements. Real PnL recorded.
...
- **REST retry**: CLOB book + Gamma fetched in parallel for verification; retry once on timeout (tenacity)
- **REST source**: CLOB orderbook (`clob.polymarket.com/book`) for live bid/ask; Gamma `outcomePrices` is midpoint and lags 25+ min on thin markets — used only as fallback
